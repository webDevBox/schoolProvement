			              ...
			              if ($results){
				           $status="true";
			                  return response()->json(["status"=>$status,"user_details" =>$results[0],"message" => "login success"], 200);
			                        ...
			                        $status="false";
			                     return response()->json(["status"=>$status,"user_details" =>json_decode("{}"),"message" => "incorrect credentials"], 404);
					                }
				}
				else if ($role=="Student")
				{
				     $results=DB::select('select st.school_id,s.school_name,st.student_id as "user_id",st.student_anonymous_name as "user_name",st.student_district as "user_district" from student st INNER JOIN school s on (st.school_id=s.school_id) where student_anonymous_name = ? and student_password = ?', [$name, $password]);
				       if ($results){
				           $status="true";
			                  return response()->json(["status"=>$status,"user_details" =>$results[0],"message" => "login success"], 200);
			                         }
...
//......................................................IOS SIGNUP APP......................................................		
		  public function userSignup(Request $request) 
		  {
			   ...
			   {
			       $status="false";
			      return response()->json(["status"=>$status,"message" => "Password should contain 5 letters"], 200);
			   }
			   else
			   {
			     if ($role=="Parent")
				{
			        ...
			        if($checkAlready)
			                  {
			                   $status="false";
			                   return response()->json(["status"=>$status,"message" => "Username Already Exist"], 200); 
			                   }
			       ...
			       if($checkAlready)
			                     {
			                        $parent_id=DB::table('parent')->where('parent_id',$id)->update(array(
			                         ...
			                         if ($parent_id){
			                          ...
			                          $status="false";
			                          return response()->json(["status"=>$status,"message" => "incorrect credentials"], 200);
					                       }
			                      }
...
//......................................IOS teacher Profile Screen APP.......................................		
		  public function teacherProfileScreen(Request $request) 
		  {
		    ...
		    if($check)
		    {    
		        foreach ($check as $records) {
                       	$teacher_absence = $records->teacher_absence;
                       	$joining_date = $records->joining_date;
                       	$date = date('Y-m-d');
                       	$teacher_district = $records->teacher_district;
                     
                 }
                 
               $check1=DB::select('select dist_id from `district` where dist_name = ? ', [$teacher_district]);
               
                foreach ($check1 as $record1) {
                    
                       	$dist_id = $record1->dist_id;
                 }  	
                 
                $check2=DB::select('select * from `schedule` where dist_id = ? ', [$dist_id]);
                
                foreach ($check2 as $record2) {
                    
                       	$summer_start = $record2->summer_start;
                       	$summer_end = $record2->summer_end;
                       	$winter_start = $record2->winter_start;
                       	$winter_end = $record2->winter_end;

                 }  	      
                    $from1 = Carbon::parse($summer_start);
                    $to1 = Carbon::parse($summer_end);
                    $summer_days= $to1->diffInWeekdays($from1);
	              
                    $from11 = Carbon::parse($winter_start);
                    $to11 = Carbon::parse($winter_end);
                    $winter_days= $to11->diffInWeekdays($from11);


		        $holidays=DB::select("SELECT COUNT(gazettedholidays.gazetted_day) as 'total_gazzeted' FROM gazettedholidays INNER join district on (gazettedholidays.dist_id= district.dist_id) INNER join teacher on (teacher.teacher_district = district.dist_name) where teacher_id = ? and district.dist_name =?", [$teacher_id,$teacher_district]);
		  	              foreach ($holidays as $record){
		  	                  $g_days = $record->total_gazzeted;
		  	              }   
		  	              
		  	              $from = Carbon::parse($joining_date);
                          $to = Carbon::parse($date);
                         $result= $to->diffInWeekdays($from);
                         $totaldays = $result-$g_days;
		  	             $alldays11 = $result-$g_days-$teacher_absence;
		  	             $alldays = $result-$g_days-$teacher_absence;
		  	             
		  	             if($summer_start>$joining_date & $summer_start<$date)
		  	             {
		  	                 if($summer_end<$date)
		  	                 {
		  	                   $a = $summer_days;
		  	                     ...
		  	                     $from111 = Carbon::parse($date);
                                 $to111 = Carbon::parse($summer_end);
                                 $diff= $to111->diffInDays($from111);
		  	                      $a =$summer_days+$diff;
		  	             ...
		  	             if($winter_start>$joining_date & $winter_start<$date)
		  	             {
		  	                 if($winter_end<$date)
		  	                 {
		  	                   $b = $winter_days;
		  	                 ...
		  	                 {
		  	                     $from1111 = Carbon::parse($date);
                                 $to1111 = Carbon::parse($winter_end);
                                 $diff1= $to1111->diffInDays($from1111);
		  	              ...
		  	              if($alldays<0){
		  	              			       ...
		  	              			       $c =$totaldays-$a-$b;
		  	              			       $alldays = $alldays11-$a-$b;
		  	       DB::table('teacher')->where('teacher_id',$teacher_id)->update(array(	'attendance_percentage'=>ROUND(($alldays/$c)*100,2)));
...
public function schoolProfileAbsentList(Request $request) {
			    ...
			    if ($results){
			        ...
			        return response()->json(["status"=>$status,"absentList" =>$results,"message" => "success"], 200);
			         ...
			         $status="true";
			         return response()->json(["status"=>$status,"absentList" =>$results,"message" => "There are no recorded absences for this school"], 200);
					 }  
                }
                else{
                   $status="false";
			       return response()->json(["status"=>$status,"absentList" =>$check,"message" => "Incorrect school id"], 400); 
                }
			   ...
			   $results="";
				...
				$results=DB::select('select id,name,info,date from habit where Parent = "true" ');
				     if($results)
				         ...
				         {
				           $status="true";
			               return response()->json(["status"=>$status,"Habits list" =>$results,"message" => "Currently no habbits available"], 200);  
				         }
				...
				else if ($Userrole=="Community Member" || $Userrole=="community")
				{
				...
				else if ($Userrole=="Principal" || $Userrole=="principal")
				{
...
public function userAddFollow(Request $request) {
			   ...
			   // die($id);
				...
				if ($followerrole=="Parent" || $followerrole=="parent")
				{
				...
				else if ($followerrole=="Student" || $followerrole=="student")
				{
				...
				else if ($followerrole=="Teacher" || $followerrole=="teacher")
				{
				...
				else if ($followerrole=="Community Member" || $followerrole=="community")
				{
				...
				else if ($followerrole=="Principal" || $followerrole=="principal")
				{
...
public function userCompletedHabits(Request $request) {
				...
				$completed=DB::select('select h.name,h.info,st.date from habit h inner join habitstatus st on (h.id=st.habit_id) where st.status="complete" AND st.user_id = ? AND st.user_role = ?', [$user_id,$user_role]);		      
                   ...
                   if($completed){
                                      ...
                                      {
                                       $status="false";
			                           return response()->json(["status"=>$status,"Completed habits" => $completed,"message" => "No completed habits yet"], 200); 
                                      }
...
public function userCurrentHabit(Request $request) {
			...
			//	$completed=DB::select('select h.id ,h.name,h.info,st.date,st.status,st.completed_days from habit h inner join habitstatus st on (h.id=st.habit_id) where st.status="complete" AND st.user_id = ? AND st.user_role = ?', [$user_id,$user_role]);		      
                    ...
                    if($current){
                                      ...
                                      {
                                       $status="false";
			                           return response()->json(["status"=>$status,"Current_habit" => json_decode("{}"),"message" => "No Current Habit yet"], 200); 
                                      }
			}
...
//.............................................................................
			   ...
			   if($feedback_to == "Teacher"){
				...
				if ($role=="Parent" || $role=="parent" )
				{
				 ...
				 if($checkAlready){
                       ...
                       {
                           $status="false";
			               return response()->json(["status"=>$status,"message" => "1 feedback in week allowed"], 200);  
                       }
				 }
				 else{
				  $results=DB::insert('INSERT INTO `feedback`(`teacher_feedback_id`,  `feedback_type`, `feedback_rating`,`feedback_review_text`,`feedback_date` , `parent_id`) VALUES (?, ?, ?, ?, ?,?)', [$id, $type, $rating, $reviews,$feedback_date, $feedbackby_id]);   
 ...
 //------------------------------------ SHOW SCHOOL TEACHER REVIEWS IOS APP-------------------------------------------
	...
	public function feedbackShow(Request $request) {
				...
				if($role=="Teacher")
				{
				    if(($user_role=="Teacher" || $user_role=="teacher")  && $user_id==$id)
				    {
			         ...
			         if ($results){
			                    ...
			                    return response()->json(["status"=>$status,"feedback_details" => $results,"message" => "Success"], 200);
			                              ...
			                              $status="true";
			                              return response()->json(["status"=>$status,"feedback_details" => $results,"message" => "There are no recored reviews for this teacher"], 200);
					               }   
				    }
				    else
				    {
				    ...
				    $results=DB::select('select schoolFeedback_review as "feedback_review_text" , feedback_date from schoolfeedback where feedback_type = "Public" AND school_id = ?', [$id]);		      
			         if ($results){
			                    ...
			                    return response()->json(["status"=>$status,"feedback_details" => $results,"message" => "Success"], 200);
			                              ...
			                              $status="true";
			                              return response()->json(["status"=>$status,"feedback_details" => $results,"message" => "There are no recored reviews for this school"], 200);
					               }
				}
				else
				{
		...
		public function reportTeacher(Request $request) {
                ...
                $results1=DB::insert('INSERT INTO `reportabsence`( `reporter_id`,`reporter_role`,`teacher_id`, `school_id`, `reportAbsence_date`, `reportAbsence_shift`,`report_time`, `attendance_status`, `teacher_claim`,`resolve`, `positiveCount`, `negativeCount`,`notification_status`)
			        ...
			        if ($results1){
			            ...
			            $getReportId=DB::select('select * from `reportabsence` where teacher_id = ? AND reportAbsence_date = ? ', [$teacher_id,$reportAbsence_date]);
                        foreach ($getReportId as $records) {
                        $report_id = $records->reportAbsence_id;
                        }
			            
			            
			           $results2=DB::insert('INSERT INTO `confirmreporter` (`reportAbsence_id`, `reporter_id`, `reporter_role`, `vote_status`) VALUES (?, ?, ?, "yes")', [$report_id,$reporter_id,$reporter_role]);
			         $status="true";
			   ...
			   $status="";
				...
				$results=DB::insert('INSERT INTO `appfeedback`(`comment`) VALUES (?)', [$comment]);
			        ...
			        if ($results){
			         ...
			         $status="false";   
			         return response()->json(["status"=>$status ,"message"=>"Feedback not added"], 200);
...
//---------------------------------------------------------------- IOS TEACHER REPORTS SHOW ------------------------------------------------------------------
 ...
 public function teacherReportsList(Request $request) {
			    ...
			    if ($results){
			        ...
			        if($check)
			        {
			            $check2=DB::select('select t.teacher_image, reportAbsence_id,reportAbsence_date from reportabsence r INNER JOIN teacher t on (r.teacher_id = t.teacher_id) where r.positiveCount >= r.negativeCount AND r.attendance_status = "absent" AND r.teacher_claim ="No Response" AND r.resolve ="no" AND r.notification_status ="expired" AND r.teacher_id = ? ORDER BY r.reportAbsence_date DESC', [$teacher_id]);
...
//----------------------------------------------------------------IOS TEACHER RESPONSE API -------------------------------------------------------------------
	...
	public function teacherResponse(Request $request) {
				...
				$status="";
				 ...
				 if($checkTeacher)
				 {
				    if($teacher_claim !="Negated Absence"){
...
public function conflictTeacherNames(Request $request)
      {
       ...
       $teacher=DB::select('select distinct t.teacher_id , t.teacher_firstName,t.teacher_lastName,t.teacher_image from  
       principal p inner join school s on (p.school_id=s.school_id) 
       inner join teacher t on (t.school_id=s.school_id) 
       inner join reportabsence r on(r.teacher_id=t.teacher_id) where t.teacher_dispute >=3 AND r.resolve="no" AND r.attendance_status ="absent" AND r.notification_status ="expired" AND r.teacher_claim ="Negated Absence" AND p.principal_id =?',[$principal_id]);
  ...
  public function conflictsList(Request $request)
      {
        ...
        $teacher_id=(int)$request->teacher_id;
        $status="";
        $teacher=DB::select('select  t.teacher_id ,  r.reportAbsence_date, r.reportAbsence_id from  teacher t
       
        inner join reportabsence r on (r.teacher_id=t.teacher_id) where t.teacher_dispute >=3 AND r.positiveCount > r.negativeCount AND r.attendance_status ="absent" AND r.resolve="no" AND t.teacher_id =?',[$teacher_id]);
        ...
        if ($teacher){
			        ...
			        return response()->json(["status"=>$status,"conflicts_list"=>$teacher,"message"=>"Success"], 200); 
			         ...
			         $status="false";   
			         return response()->json(["status"=>$status,"conflicts_list"=>$teacher,"message"=>"No conflicts yet"], 200);
					 }
...
//.............................................................................
            ...
            public function pushNotifications(Request $request) {
                ...
                $results=DB::select('SELECT * FROM `notification` WHERE (dist = ? and school = "all" AND (user like "%Students%" OR user like "Students%" OR user like "%Students")) 
                        OR (dist = "all" and school = "all" AND (user like "%Students%" OR user like "Students%" OR user like "%Students")) 
                        OR (dist = ? AND school = ? AND (user like "%Students%" OR user like "Students%" OR user like "%Students"))', [$dist,$dist, $school]);
                   
                     
                   }
                   else if($user == "Parents")
                   {
                   $results=DB::select('SELECT * FROM `notification` WHERE (dist = ? and school = "all" AND (user like "%Parents%" OR user like "Parents%" OR user like "%Parents")) 
                        OR (dist = "all" and school = "all" AND (user like "%Parents%" OR user like "Parents%" OR user like "%Parents")) 
                        OR (dist = ? AND school = ? AND (user like "%Parents%" OR user like "Parents%" OR user like "%Parents"))', [$dist,$dist, $school]);
                    ...
                    else if($user == "Community Members")
                   {
                       ...
                       $results=DB::select('SELECT * FROM `notification` WHERE (dist = ? and school = "all" AND (user like "%Community Members%" OR user like "Community Members%" OR user like "%Teachers")) 
                        OR (dist = "all" and school = "all" AND (user like "%Community Members%" OR user like "Community Members%" OR user like "%Community Members")) 
                        OR (dist = ? AND school = ? AND (user like "%Community Members%" OR user like "Community Members%" OR user like "%Community Members"))', [$dist,$dist, $school]);
                   
                   }
                    else if($user == "Teachers")
                   {

                        $results=DB::select('SELECT * FROM `notification` WHERE (dist = ? and school = "all" AND (user like "%Teachers%" OR user like "Teachers%" OR user like "%Teachers")) 
                        OR (dist = "all" and school = "all" AND (user like "%Teachers%" OR user like "Teachers%" OR user like "%Teachers")) 
                        OR (dist = ? AND school = ? AND (user like "%Teachers%" OR user like "Teachers%" OR user like "%Teachers"))', [$dist,$dist, $school]);
                   ...
                   { 
                        ...
                        $results=DB::select('SELECT * FROM `notification` WHERE (dist = ? and school = "all" AND (user like "%Principals%" OR user like "Principals%" OR user like "%Principals")) 
                        OR (dist = "all" and school = "all" AND (user like "%Principals%" OR user like "Principals%" OR user like "%Principals")) 
                        OR (dist = ? AND school = ? AND (user like "%Principals%" OR user like "Principals%" OR user like "%Principals"))', [$dist,$dist, $school]);
                   }
...
//.............................................................................
			...
			//......................................TEACHER ATTENDENCE PERCENTAGE.......................................		
		  public function getteacherpercentage(Request $request) 
		  {
                	$teacher_id=(int)$request->teacher_id;		    
               ...
               $check1=DB::select('select dist_id from `district` where dist_name = ? ', [$teacher_district]);
		        ...
		        $holidays=DB::select("SELECT COUNT(gazettedholidays.gazetted_day) as 'total_gazzeted' FROM gazettedholidays INNER join district on (gazettedholidays.dist_id= district.dist_id) INNER join teacher on (teacher.teacher_district = district.dist_name) where teacher_id = ? and district.dist_name =?", [$teacher_id,$teacher_district]);
		  	       ...
		  	       $b = $winter_days+$diff1;
		  	              			       ...
		  	              			       $c =$totaldays-$a-$b;
		  	              			       $alldays = $alldays11-$a-$b;
		  	              			       DB::table('teacher')->where('teacher_id',$teacher_id)->update(array(	
			                                         'attendance_percentage'=>ROUND(($alldays/$c)*100,2)));

			          return response()->json(["status"=>$status,"percentage" =>ROUND(($alldays/$c)*100,2),"total days"=>$c,"g days"=>$g_days,"workin days"=>$result,"summer"=>$summer_days,"winter"=>$winter_days,"all days"=>$alldays,"message" => "success"], 200);
			}
			 else {
			       $status="false";
			       return response()->json(["status"=>$status,"percentage" =>$alldays,"message" => "incorrect teacher id"], 200);
				  }
