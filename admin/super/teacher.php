...
else if(isset($_POST['file']))
{
  ...
  foreach ($objPHPExcel->getWorksheetIterator() as $worksheet)
  {
   ...
   for($row9=2; $row9<=$highestRow; $row9++)
   {
    ...
    $country=mysqli_real_escape_string($conn, $worksheet->getCellByColumnAndRow(3, $row9)->getValue());
    $district1 = mysqli_real_escape_string($conn, $worksheet->getCellByColumnAndRow(4, $row9)->getValue());
    $schoolName = mysqli_real_escape_string($conn, $worksheet->getCellByColumnAndRow(5, $row9)->getValue());
    $Join1 = date("Y-m-d"); 
